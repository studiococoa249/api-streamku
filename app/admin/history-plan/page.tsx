...
export default async function AdminHistoryPlanPage() {
  ...
  const successCount = histories.filter((h) => h.statusPayment === "Success").length;
  const pendingCount = histories.filter((h) => h.statusPayment === "Pending").length;
  const failedCount = histories.filter(
    (h) => h.statusPayment === "Error" || h.statusPayment === "Cancel" || h.statusPayment === "Expired"
  ).length;
